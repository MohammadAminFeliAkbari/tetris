...
public class Board {
    ...
    public void print_block_colored() {
        Random rnd = new Random();
        int number = rnd.nextInt(3);

        switch (number) {
            case 0:
                System.out.print("\u001B[43m\u001B[30m  \u001B[0m");
                break;
            case 1:
                    ...
                    for (int j = this.soton - 2; j > 0; j--)
                        if (arr[i][j] == 2) {
                            if (arr[i + 1][j] != 2 && arr[i + 1][j + 1] != 2 && arr[i + 1][j - 1] != 2)
                                arr[i][j] = 1;
                            if (arr[i + 1][j] != 2 && arr[i + 1][j - 1] != 2 && arr[i][j + 1] != 2 && arr[i][j - 1] != 2)
                                arr[i][j] = 1;
                        }
...
//    }
    ...
    public void rotate_180() {
        ...
        if (number_one_two_if != number_one_first_if) {
            for (int i = this.satr - 2; i > -1; i--)
                for (int j = this.soton - 2; j > 0; j--)
                    if (arr[i][j] == 1)
                        arr[i][j] = 0;
    ...
    public void rotate_90() {
        ...
        for (int i = this.satr - 2; i > -1; i--)
            if (arr[i][0] == 3) {
                for (int j = this.satr - 2; j > -1; j--)
                    for (int k = this.soton - 2; k > 0; k--)
                        if (arr[j][k] == 1)
                            arr[j][k] = 0;
